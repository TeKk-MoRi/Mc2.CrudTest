...
namespace Mc2.CrudTest.Service.Base
{
    public class BaseService<T, TType> : IBaseService<T, TType>
        where T : class
        where TType : IComparable
    {
        protected readonly CrudTestContext _dbContext;

        protected BaseService(CrudTestContext context)
        {
            _dbContext = context;
        }
    ...
    public class BaseService<T> : BaseService<T, int>
        where T : class
    {
        protected BaseService(CrudTestContext context) : base(context)
        {
        }
    }
}
